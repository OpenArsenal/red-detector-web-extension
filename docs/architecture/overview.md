# Architecture overview after phase 12

This overview gives maintainers a single map for the current extension and the migration path toward the event-based detection engine. It describes verified repository shape after phases 1 through 12, not the earlier small-registry MVP narrative.
...
| Candidates | `src/lib/candidates/**` | Evidence entries can be aggregated into deterministic candidate batches without running graph refinement. | Compare candidate confidence and version selection with current detector output before runtime cutover. |
...
- `PageSignals` remains the detector input; normalized observations, pattern-match events, evidence entries, and evidence candidates now exist beside it for future replay work.
...
The following decisions are intentionally not resolved by phase 12.
...
| Explanation output in popup | Not shown yet. | Candidate/evidence contracts now exist, but explanation view-models and disclosure rules still need a separate UI phase. |
