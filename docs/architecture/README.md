...
These docs are the current maintainer map for the Red Detector migration. They describe the branch after phases 1 through 11, where the extension still uses the existing active-tab flow but now has explicit seams for contracts, collectors, lifecycle, graph reasoning, popup view state, and shared tests.
