...
The storage-backed runtime now separates first-result readiness from deeper enrichment. Active-tab analysis can use the bootstrap registry for the first visible detector snapshot, then load the enrichment registry as a follow-up. The popup receives bounded detector frames through storage revisions and receives terminal enrichment states when deeper evidence completes, fails, times out, is skipped, or is not needed. This improves the perceived startup path without changing the invariant that replay explanations describe the complete candidate graph.
...
| streaming snapshot contracts | Complete | `src/lib/contracts/detection-session.ts` and `src/lib/storage/detection-snapshots.ts` | Popup-visible detection state can be written as monotonic session revisions and promoted to an origin-level latest record. | Popup subscription, content-side snapshot emission, and background enrichment promotion are wired into the runtime. |
